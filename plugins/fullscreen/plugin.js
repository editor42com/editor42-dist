...
(function () {
    ...
    const attr = 'data-editor42-mobile-fullscreen-style';
    ...
    const clobberStyles = (dom, container, editorBody) => {
      const gatherSiblings = element => {
        return siblings(element, '*:not(.editor42-t-silver-sink)');
      };
    ...
    const toggleFullscreen = (editor, fullscreenState) => {
      ...
      const sinkContainerS = nextSibling(editorContainerS).filter(elm => isHTMLElement(elm) && has(elm, 'editor42-t-silver-sink'));
      ...
      const handleClasses = handler => {
        handler(body, 'editor42-t-fullscreen');
        handler(documentElement, 'editor42-t-fullscreen');
        handler(editorContainer, 'editor42-t-fullscreen');
        getShadowRoot(editorContainerS).map(root => getShadowHost(root).dom).each(host => {
          handler(host, 'editor42-t-fullscreen');
          handler(host, 'editor42-t-shadowhost');
        });
      };
      const cleanup = () => {
        if (isTouch) {
    ...
    const register$1 = (editor, fullscreenState) => {
      editor.addCommand('editor42FullScreen', () => {
        toggleFullscreen(editor, fullscreenState);
      });
    };
    ...
    const register = (editor, fullscreenState) => {
      const onAction = () => editor.execCommand('editor42FullScreen');
    ...
    var Plugin = () => {
      global$2.add('fullscreen', editor => {
        ...
        editor.addShortcut('Meta+Shift+F', '', 'editor42FullScreen');
        return get$5(fullscreenState);
      });
    };
