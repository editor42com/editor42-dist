...
(function () {
    ...
    var global$3 = editor42.util.Tools.resolve('editor42.PluginManager');
    ...
    var global$2 = editor42.util.Tools.resolve('editor42.util.Tools');
    ...
    var global$1 = editor42.util.Tools.resolve('editor42.Env');
    ...
    const applyTableCellStyle = (editor, style) => value => {
      editor.execCommand('editor42TableApplyCellStyle', false, { [style]: value });
    };
    ...
    const changeRowHeader = editor => () => {
      const currentType = editor.queryCommandValue('editor42TableRowType');
      const newType = currentType === 'header' ? 'body' : 'header';
      editor.execCommand('editor42TableRowType', false, { type: newType });
    };
    const changeColumnHeader = editor => () => {
      const currentType = editor.queryCommandValue('editor42TableColType');
      const newType = currentType === 'th' ? 'td' : 'th';
      editor.execCommand('editor42TableColType', false, { type: newType });
    };
    ...
    const applyStructureData$1 = (editor, data) => {
      editor.execCommand('editor42TableCellType', false, {
        type: data.celltype,
        no_events: true
      });
    };
    ...
    const applyStructureData = (editor, data) => {
      editor.execCommand('editor42TableRowType', false, {
        type: data.type,
        no_events: true
      });
    };
    ...
    const onSubmitTableForm = (editor, tableElm, oldData, api) => {
      ...
      editor.undoManager.transact(() => {
        if (!tableElm) {
          ...
          editor.execCommand('editor42InsertTable', false, {
            rows,
            columns: cols
          });
        ...
        if (size(modifiedData) > 0) {
          ...
          if (captionElm && !data.caption || !captionElm && data.caption) {
            editor.execCommand('editor42TableToggleCaption');
          }
    ...
    const open = (editor, insertNewTable) => {
      ...
      if (classes.length > 0) {
        if (data.class) {
          data.class = data.class.replace(/\s*editor42-m\-item\-table\s*/g, '');
        }
      }
      const generalPanel = {
        type: 'grid',
    ...
    const registerCommands = editor => {
      ...
      each$1({
        editor42TableProps: curry(open, editor, false),
        editor42TableRowProps: curry(open$1, editor),
        editor42TableCellProps: curry(open$2, editor),
        editor42InsertTableDialog: curry(open, editor, true)
    ...
    const getSelectionTargets = editor => {
      ...
      const onSetupTableRowHeaders = onSetupTableHeaders('editor42TableRowType', 'header');
      const onSetupTableColumnHeaders = onSetupTableHeaders('editor42TableColType', 'th');
    ...
    const addButtons = (editor, selectionTargets) => {
      ...
      addToggleButtonIfRegistered('tablecaption', {
        ...
        icon: 'table-caption',
        command: 'editor42TableToggleCaption',
        onSetup: selectionTargets.onSetupTableWithCaption
      });
      addToggleButtonIfRegistered('tablerowheader', {
        tooltip: 'Row header',
        icon: 'table-top-header',
        command: 'editor42TableRowType',
    ...
    const addMenuItems = (editor, selectionTargets) => {
      ...
      if (!hasTableGrid(editor)) {
        editor.ui.registry.addMenuItem('inserttable', {
          text: 'Table',
          icon: 'table',
          onAction: cmd('editor42InsertTableDialog'),
      ...
      if (tableClassList.length !== 0 && editor.queryCommandSupported('editor42TableToggleClass')) {
        editor.ui.registry.addNestedMenuItem('tableclass', {
          icon: 'table-classes',
          text: 'Table styles',
          getSubmenuItems: () => buildMenuItems(editor, tableClassList, 'tableclass', value => editor.execCommand('editor42TableToggleClass', false, value)),
          onSetup: selectionTargets.onSetupTable
        });
      }
      const tableCellClassList = filterNoneItem(getCellClassList(editor));
      if (tableCellClassList.length !== 0 && editor.queryCommandSupported('editor42TableCellToggleClass')) {
        editor.ui.registry.addNestedMenuItem('tablecellclass', {
          icon: 'table-cell-classes',
          text: 'Cell styles',
          getSubmenuItems: () => buildMenuItems(editor, tableCellClassList, 'tablecellclass', value => editor.execCommand('editor42TableCellToggleClass', false, value)),
          onSetup: selectionTargets.onSetupCellOrRow
        });
      }
      if (editor.queryCommandSupported('editor42TableApplyCellStyle')) {
