...
(function () {
    ...
    var global$2 = editor42.util.Tools.resolve('editor42.Env');

    var global$1 = editor42.util.Tools.resolve('editor42.util.Tools');
    ...
    var global = editor42.util.Tools.resolve('editor42.dom.TreeWalker');
    ...
    const mark = (matches, replacementNode) => {
      eachr(matches, (match, idx) => {
        eachr(match, pos => {
          ...
          set(wrapper, 'data-editor42-index', idx);
    ...
    const findAndMarkInSelection = (dom, pattern, selection, replacementNode) => {
      ...
      const nodes = dom.select('td[data-editor42-selected],th[data-editor42-selected]');
    ...
    const getElmIndex = elm => {
      return elm.getAttribute('data-editor42-index');
    };
    const markAllMatches = (editor, currentSearchState, pattern, inSelection) => {
      const marker = editor.dom.create('span', { 'data-editor42-bogus': 1 });
      marker.className = 'editor42-m-match-marker';
    ...
    const moveSelection = (editor, currentSearchState, forward) => {
      ...
      dom.removeClass(findSpansByIndex(editor, searchState.index), 'editor42-m-match-marker-selected');
      const spans = findSpansByIndex(editor, testIndex);
      if (spans.length) {
        dom.addClass(findSpansByIndex(editor, testIndex), 'editor42-m-match-marker-selected');
    ...
    const replace = (editor, currentSearchState, text, forward, all) => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (all || matchIndex === searchState.index) {
          ...
          nodes[i].setAttribute('data-editor42-index', String(currentMatchIndex - 1));
        }
      }
      currentSearchState.set({
        ...searchState,
