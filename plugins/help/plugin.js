...
(function () {
    ...
    const register$2 = (editor, dialogOpener) => {
      editor.addCommand('editor42Help', dialogOpener);
    };
    ...
    var global$3 = editor42.util.Tools.resolve('editor42.Resource');

    var global$2 = editor42.util.Tools.resolve('editor42.util.I18n');

    const pLoadHtmlByLangCode = (baseUrl, langCode) => global$3.load(`editor42.html-i18n.help-keynav.${ langCode }`, `${ baseUrl }/js/i18n/keynav/${ langCode }.js`);
    ...
    const tab$1 = editor => {
      ...
      const getPluginData = (editor, key) => find(urls, x => {
        ...
        const html = makeLink({
          name: x.name,
          url: `https://editor42.com/plugins/${ x.slug }`
        });
        return {
          name: x.name,
          html
    ...
    var global = editor42.util.Tools.resolve('editor42.EditorManager');

    const tab = () => {
      const getVersion = minor => minor.indexOf('@') === 0 ? 'X.X.X' : minor;
      const version = getVersion(global.minorVersion);
      const changeLogLink = '<a data-alloy-tabstop="true" tabindex="-1" href="https://editor42.com/docs/changelog" rel="noopener" target="_blank">Editor42 ' + version + '</a>';
      const htmlPanel = {
        type: 'htmlpanel',
    ...
    var Plugin = () => {
      global$4.add('help', (editor, pluginUrl) => {
        ...
        editor.shortcuts.add('Alt+0', 'Open help dialog', 'editor42Help');
