...
(function () {
    ...
    const charMapToSelector = charMap => {
      ...
      each(charMap, value => {
        ...
        selector += 'span.editor42-m-' + value;
      });
      return selector;
    };
    ...
    const nbspClass = 'editor42-m-nbsp';

    const getRaw = element => element.dom.contentEditable;

    const wrapCharWithSpan = value => '<span data-editor42-bogus="1" class="editor42-m-' + charMap[value] + '">' + value + '</span>';

    const isWrappedNbsp = node => node.nodeName.toLowerCase() === 'span' && node.classList.contains('editor42-m-nbsp-wrap');
    const isMatch = n => {
    ...
    const register$2 = (editor, toggleState) => {
      editor.addCommand('editor42VisualChars', () => {
    ...
    const register = (editor, toggleState) => {
      const onAction = () => editor.execCommand('editor42VisualChars');
