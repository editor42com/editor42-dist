...
(function () {
    ...
    var global = editor42.util.Tools.resolve('editor42.Resource');

    const DEFAULT_ID = 'editor42.plugins.emoticons';
    ...
    const register$1 = (editor, database) => {
      editor.addCommand('editor42Emoticons', () => open(editor, database));
    };
    ...
    const setup = editor => {
      editor.on('PreInit', () => {
        editor.parser.addAttributeFilter('data-emoticon', nodes => {
          each$1(nodes, node => {
            node.attr('data-editor42-resize', 'false');
            node.attr('data-editor42-placeholder', '1');
          });
        });
      });
    };
    ...
    const register = editor => {
      const onAction = () => editor.execCommand('editor42Emoticons');
