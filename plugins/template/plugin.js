...
(function () {
    ...
    var global$1 = editor42.util.Tools.resolve('editor42.html.Serializer');
    ...
    const insertTemplate = (editor, _ui, html) => {
      ...
      const n = dom.select('.editor42Tmpl', el);
      if (n && n.length > 0) {
        el = dom.create('div');
      ...
      editor.execCommand('editor42InsertContent', false, el.innerHTML);
      editor.addVisual();
    };
    ...
    const open = (editor, templateList) => {
      ...
      const onSubmit = templates => api => {
        ...
        findTemplate(templates, data.template).each(t => {
          getTemplateContent(t).then(previewHtml => {
            editor.execCommand('editor42InsertTemplate', false, previewHtml);
            api.close();
          }).catch(() => {
            api.setEnabled('save', false);
            loadFailedAlert(api);
    ...
    const register$1 = editor => {
      editor.addCommand('editor42InsertTemplate', curry(insertTemplate, editor));
      editor.addCommand('editor42Template', createTemplateList(editor, showDialog(editor)));
    };
    ...
    const setup = editor => {
      editor.on('PreProcess', o => {
        ...
        global$2.each(dom.select('div', o.node), e => {
          if (dom.hasClass(e, 'editor42Tmpl')) {
    ...
    const register = editor => {
      const onAction = () => editor.execCommand('editor42Template');
