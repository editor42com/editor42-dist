...
(function () {
    ...
    var global$7 = editor42.util.Tools.resolve('editor42.PluginManager');
    ...
    var global$6 = editor42.util.Tools.resolve('editor42.util.Tools');

    var global$5 = editor42.util.Tools.resolve('editor42.dom.DOMUtils');

    var global$4 = editor42.util.Tools.resolve('editor42.html.DomParser');

    const DOM$1 = global$5.DOM;
    const trimPx = value => value.replace(/px$/, '');
    const getEditor42EmbedData = node => {
      const style = node.attr('style');
      const styles = style ? DOM$1.parseStyle(style) : {};
      return {
        type: 'editor42-embed-iri',
        source: node.attr('data-editor42-embed-iri'),
        altsource: '',
        poster: '',
    ...
    const htmlToData = (html, schema) => {
      ...
      for (let node = rootNode; node; node = node.walk()) {
        if (node.type === 1) {
          ...
          if (node.attr('data-editor42-embed-iri')) {
            data = getEditor42EmbedData(node);
            break;
          } else {
            ...
            if (name === 'iframe' || name === 'object' || name === 'embed' || name === 'video' || name === 'audio') {
              ...
              data = global$6.extend(node.attributes.map, data);
            }
            if (name === 'source') {
              if (!data.source) {
    ...
    var global$3 = editor42.util.Tools.resolve('editor42.html.Node');

    var global$2 = editor42.util.Tools.resolve('editor42.html.Serializer');

    const Parser = (schema, settings = {}) => global$4({
      forced_root_block: false,
      validate: false,
    ...
    const DOM = global$5.DOM;
    const addPx = value => /^[0-9.]+$/.test(value) ? value + 'px' : value;
    const updateEditor42Embed = (data, node) => {
      const style = node.attr('style');
    ...
    const updateHtml = (html, data, updateAll, schema) => {
              ...
              case 'video':
                ...
                for (let index = numSources; index < 2; index++) {
                  if (data[sources[index]]) {
                    const source = new global$3('source', 1);
      ...
      return global$2({}, schema).serialize(rootNode);
    };
    ...
    const matchPattern = url => {
      ...
      if (patterns.length > 0) {
        return global$6.extend({}, patterns[0], { url: getUrl(patterns[0], url) });
      } else {
        return null;
      }
    };
    ...
    const dataToHtml = (editor, dataIn) => {
      var _a;
      const data = global$6.extend({}, dataIn);
      if (!data.source) {
        global$6.extend(data, htmlToData((_a = data.embed) !== null && _a !== void 0 ? _a : '', editor.schema));
        if (!data.source) {
          return '';
        }
      }
    ...
    const isMediaElement = element => element.hasAttribute('data-editor42-object') || element.hasAttribute('data-editor42-embed-iri');
    const setup$2 = editor => {
      editor.on('click keyup touchend', () => {
        const selectedNode = editor.selection.getNode();
        if (selectedNode && editor.dom.hasClass(selectedNode, 'editor42-m-preview-object')) {
          if (editor.dom.getAttrib(selectedNode, 'data-editor42-selected')) {
            selectedNode.setAttribute('data-editor42-selected', '2');
          }
        }
      });
      editor.on('ObjectResized', e => {
        const target = e.target;
        if (target.getAttribute('data-editor42-object')) {
          let html = target.getAttribute('data-editor42-html');
          if (html) {
            html = unescape(html);
            target.setAttribute('data-editor42-html', escape(updateHtml(html, {
    ...
    const selectPlaceholder = (editor, beforeObjects) => {
      const afterObjects = editor.dom.select('*[data-editor42-object]');
    ...
    const handleInsert = (editor, html) => {
      const beforeObjects = editor.dom.select('*[data-editor42-object]');
      editor.insertContent(html);
      selectPlaceholder(editor, beforeObjects);
      editor.nodeChanged();
    };
    const isEmbedIframe = (url, mediaDataType) => isNonNullable(mediaDataType) && mediaDataType === 'editor42-embed-iri' && isNonNullable(matchPattern(url));
    ...
    const register$1 = editor => {
      ...
      editor.addCommand('editor42Media', showDialog$1);
    };

    var global$1 = editor42.util.Tools.resolve('editor42.util.URI');
    ...
    var global = editor42.util.Tools.resolve('editor42.Env');
    ...
    const createPlaceholderNode = (editor, node) => {
      const name = node.name;
      const placeHolder = new global$3('img', 1);
      retainAttributesAndInnerHtml(editor, node, placeHolder);
      setDimensions(node, placeHolder, {});
      placeHolder.attr({
        'style': node.attr('style'),
        'src': global.transparentSrc,
        'data-editor42-object': name,
        'class': 'editor42-m-object editor42-m-object-' + name
      });
      return placeHolder;
    };
    const createPreviewNode = (editor, node) => {
      var _a;
      const name = node.name;
      const previewWrapper = new global$3('span', 1);
      previewWrapper.attr({
        'contentEditable': 'false',
        'style': node.attr('style'),
        'data-editor42-object': name,
        'class': 'editor42-m-preview-object editor42-m-object-' + name
      });
      retainAttributesAndInnerHtml(editor, node, previewWrapper);
      const styles = editor.dom.parseStyle((_a = node.attr('style')) !== null && _a !== void 0 ? _a : '');
      const previewNode = new global$3(name, 1);
      ...
      if (name === 'iframe') {
        ...
        const sanitizedHtml = previewWrapper.attr('data-editor42-html');
        if (isNonNullable(sanitizedHtml)) {
          appendNodeContent(editor, name, previewNode, unescape(sanitizedHtml));
        }
      }
      const shimNode = new global$3('span', 1);
      shimNode.attr('class', 'editor42-m-shim');
    ...
    const retainAttributesAndInnerHtml = (editor, sourceNode, targetNode) => {
      ...
      while (ai--) {
        ...
        if (attrName !== 'width' && attrName !== 'height' && attrName !== 'style' && !startsWith(attrName, 'data-editor42-')) {
          if (attrName === 'data' || attrName === 'src') {
            attrValue = editor.convertURL(attrValue, attrName);
          }
          targetNode.attr('data-editor42-p-' + attrName, attrValue);
        }
      }
      const serializer = global$2({ inner: true }, editor.schema);
      const tempNode = new global$3('div', 1);
      each$1(sourceNode.children(), child => tempNode.append(child));
      const innerHtml = serializer.serialize(tempNode);
      if (innerHtml) {
        targetNode.attr('data-editor42-html', escape(innerHtml));
        targetNode.empty();
      }
    };
    ...
    const isWithinEmbedWrapper = node => {
      ...
      while (tempNode = tempNode.parent) {
        if (tempNode.attr('data-editor42-embed-iri') || isPageEmbedWrapper(tempNode)) {
          return true;
        }
      }
      return false;
    ...
    const placeHolderConverter = editor => nodes => {
      ...
      while (i--) {
        ...
        if (node.parent.attr('data-editor42-object')) {
          continue;
        }
    ...
    const urlAttrs = global$6.makeMap('src,href,data,background,action,formaction,poster,xlink:href');
    const placeholderPrefix = 'data-editor42-p-';
    const setup$1 = editor => {
      editor.on('PreInit', () => {
        ...
        serializer.addAttributeFilter('data-editor42-object', (nodes, name) => {
          var _a;
          ...
          while (i--) {
            ...
            const realElm = new global$3(realElmName, 1);
            if (realElmName !== 'audio') {
              const className = node.attr('class');
              if (className && className.indexOf('editor42-m-preview-object') !== -1 && node.firstChild) {
                realElm.attr({
            ...
            while (ai--) {
              const attrName = attribs[ai].name;
              if (attrName.indexOf(placeholderPrefix) === 0) {
                const unprefixed = attrName.substr(placeholderPrefix.length);
                const attrValue = attribs[ai].value;
                if (/^on/i.test(unprefixed) || unprefixed.toLowerCase() === 'srcdoc') {
                  continue;
                }
                if (unprefixed in urlAttrs && !global$1.isDomSafe(attrValue, realElmName, uriOptions)) {
                  continue;
                }
                realElm.attr(unprefixed, attrValue);
              }
            }
            const innerHtml = node.attr('data-editor42-html');
            if (innerHtml) {
      ...
      editor.on('SetContent', () => {
        ...
        each$1(dom.select('span.editor42-m-preview-object'), elm => {
          if (dom.select('span.editor42-m-shim', elm).length === 0) {
            dom.add(elm, 'span', { class: 'editor42-m-shim' });
          }
        });
      });
    };
    ...
    const setup = editor => {
      editor.on('ResolveName', e => {
        let name;
        if (e.target.nodeType === 1 && (name = e.target.getAttribute('data-editor42-object'))) {
          e.name = name;
        }
      });
    };
    ...
    const register = editor => {
      const onAction = () => editor.execCommand('editor42Media');
      editor.ui.registry.addToggleButton('media', {
        ...
        onSetup: buttonApi => {
          ...
          const unbindSelectorChanged = selection.selectorChangedWithUnbind('img[data-editor42-object],span[data-editor42-object],div[data-editor42-embed-iri]', buttonApi.setActive).unbind;
