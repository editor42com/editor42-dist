...
(function () {
    ...
    var global$1 = editor42.util.Tools.resolve('editor42.ModelManager');
    ...
    const removeDataStyle = table => {
      remove$7(table, 'data-editor42-style');
      const removeStyleAttribute = element => remove$7(element, 'data-editor42-style');
    ...
    const strSelected = 'data-editor42-selected';
    const strSelectedSelector = 'td[' + strSelected + '],th[' + strSelected + ']';
    const strAttributeSelector = '[' + strSelected + ']';
    const strFirstSelected = 'data-editor42-first-selected';
    const strFirstSelectedSelector = 'td[' + strFirstSelected + '],th[' + strFirstSelected + ']';
    const strLastSelected = 'data-editor42-last-selected';
    ...
    const TableActions = (editor, resizeHandler, cellSelectionHandler) => {
      ...
      const setSelectionFromAction = (table, result) => result.cursor.fold(() => {
        ...
        return head(cells).filter(inBody).map(firstCell => {
          ...
          set$2(firstCell, 'data-editor42-selected', '1');
          return rng;
        });
      }, cell => {
    ...
    const insert = (editor, columns, rows, colHeaders, rowHeaders) => {
      ...
      editor.undoManager.ignore(() => {
        ...
        set$2(table, 'data-editor42-id', '__editor42');
        const html = getOuter(table);
        editor.insertContent(html);
        editor.addVisual();
      });
      return descendant(getBody(editor), 'table[data-editor42-id="__editor42"]').map(table => {
        ...
        removeDataStyle(table);
        remove$7(table, 'data-editor42-id');
    ...
    const insertTable = (editor, rows, columns, options = {}) => {
      ...
      if (checkInput(rows) && checkInput(columns)) {
        ...
        console.error('Invalid values for editor42InsertTable - rows and columns values are required to insert a table.');
        return null;
      }
    };

    var global = editor42.util.Tools.resolve('editor42.FakeClipboard');

    const tableTypeBase = 'x-editor42/dom-table-';
    ...
    const registerCommands = (editor, actions) => {
      ...
      each$1({
        editor42TableSplitCells: () => actOnSelection(actions.unmergeCells),
        editor42TableMergeCells: () => actOnSelection(actions.mergeCells),
        editor42TableInsertRowBefore: () => actOnSelection(actions.insertRowsBefore),
        editor42TableInsertRowAfter: () => actOnSelection(actions.insertRowsAfter),
        editor42TableInsertColBefore: () => actOnSelection(actions.insertColumnsBefore),
        editor42TableInsertColAfter: () => actOnSelection(actions.insertColumnsAfter),
        editor42TableDeleteCol: () => actOnSelection(actions.deleteColumn),
        editor42TableDeleteRow: () => actOnSelection(actions.deleteRow),
        editor42TableCutCol: () => copyColSelection().each(selection => {
          setColumns(selection);
          actOnSelection(actions.deleteColumn);
        }),
        editor42TableCutRow: () => copyRowSelection().each(selection => {
          setRows(selection);
          actOnSelection(actions.deleteRow);
        }),
        editor42TableCopyCol: () => copyColSelection().each(selection => setColumns(selection)),
        editor42TableCopyRow: () => copyRowSelection().each(selection => setRows(selection)),
        editor42TablePasteColBefore: () => pasteOnSelection(actions.pasteColsBefore, getColumns),
        editor42TablePasteColAfter: () => pasteOnSelection(actions.pasteColsAfter, getColumns),
        editor42TablePasteRowBefore: () => pasteOnSelection(actions.pasteRowsBefore, getRows),
        editor42TablePasteRowAfter: () => pasteOnSelection(actions.pasteRowsAfter, getRows),
        editor42TableDelete: eraseTable,
        editor42TableCellToggleClass: toggleTableCellClass,
        editor42TableToggleClass: toggleTableClass,
        editor42TableToggleCaption: toggleCaption,
        editor42TableSizingMode: (_ui, sizing) => setSizingMode(sizing),
        editor42TableCellType: actOnType(type => type === 'th' ? actions.makeCellsHeader : actions.unmakeCellsHeader),
        editor42TableColType: actOnType(type => type === 'th' ? actions.makeColumnsHeader : actions.unmakeColumnsHeader),
        editor42TableRowType: actOnType(type => {
          switch (type) {
          case 'header':
      ...
      editor.addCommand('editor42InsertTable', (_ui, args) => {
        insertTable(editor, args.rows, args.columns, args.options);
      });
      editor.addCommand('editor42TableApplyCellStyle', (_ui, args) => {
    ...
    const registerQueryCommands = (editor, actions) => {
      ...
      each$1({
        editor42TableRowType: () => lookupOnSelection(actions.getTableRowType),
        editor42TableCellType: () => lookupOnSelection(actions.getTableCellType),
        editor42TableColType: () => lookupOnSelection(actions.getTableColType)
    ...
    const hasInternalTarget = e => !has(SugarElement.fromDom(e.target), 'editor42-snooker-resizer-bar');
    ...
    const styles$1 = css('editor42-dragster');
    ...
    const styles = css('editor42-snooker');
    ...
    const isResizable = elm => get$b(elm, 'data-editor42-resize') !== 'false';
    ...
    const TableResizeHandler = editor => {
      ...
      editor.on('ObjectResizeStart', e => {
        ...
        if (isTable(targetElm)) {
          ...
          each$2(editor.dom.select('.editor42-m-clonedresizable'), clone => {
            editor.dom.addClass(clone, 'editor42-m-' + getTableColumnResizingBehaviour(editor) + '-columns');
          });
